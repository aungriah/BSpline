 ...
 */
class BSplineParamCurve2D
{
public:
	BSplineParamCurve2D(std::vector<double> &x, std::vector<double> &y, int preferredDegree=3, double smoothing=0.0);
	// BSplineParamCurve2D(std::vector<double> &knotX, std::vector<double> &coefs, int degree);
	// BSplineParamCurve2D(const std::string &filename);
    ...
    void curve_eval(std::vector<double> &ts, std::vector<double> &pts);
    void curv_der(std::vector<double> &pts, std::vector<double> &d);
